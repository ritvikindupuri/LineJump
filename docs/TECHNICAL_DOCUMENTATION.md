...
1. **SSRF Guarding (`src/lib/ssrf-guard.ts`):** Validates the _resolved IP_ (not the hostname) against a public-only allowlist, pins the connection to that IP to block DNS rebinding, enforces HTTPS, and re-validates every redirect hop.
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SSRF network risks**          | The live-fetch path (`ssrf-guard.ts`) validates the _resolved IP_ against a public-only allowlist, resolves and checks every DNS record, pins the connection to the validated IP to defeat DNS rebinding, enforces HTTPS, and re-validates every redirect hop. Hostname-string matching is not relied upon. |
...
| **SSRF Guard**                  | `src/lib/ssrf-guard.test.ts`       | Covers the public-IP allowlist (loopback, private, link-local, ULA, CGNAT, IPv4-mapped IPv6), regression cases for the original bypasses (a public hostname resolving to an internal IP, encoded and IPv6 loopback literals), HTTPS enforcement, credential rejection, and redirect-target resolution. |
